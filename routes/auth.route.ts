...
router.get('/upload',verifyToken,async (req: any, res: any) => {
...
router.post('/enregistrer', verifyToken, async (req: any, res: any) => {
  try {
...
router.post('/enregistrerExamen', verifyToken, async (req: any, res: any) => {
  try {
    const { title, subject, dueDate, semestre, className, url, type } = req.body;
    const { id } = req.user;
...
///RECOVER FILE
router.get('/recuperer', verifyToken, async (req: any, res: any) => {
  try {
    const { id } = req.user;
...
router.get('/recupererExamen', verifyToken, async (req: any, res: any) => {
  try {
    const { id } = req.user;
...
router.get('/dashboard-stats', async (req, res) => {
  try {
    ...
    `);
    const eleveActive = parseInt((results[0] as any).eleve_active, 10);
...
router.get('/key-indicators', async (req, res) => {
  try {
    ...
    const [attendance] = await sequelize.query(`
      ...
      FROM presence;
    `, { type: QueryTypes.SELECT }) as [{ taux: number }];
...
router.get('/classes', verifyToken, async (req: any, res: any) => {
  try {
...
router.get('/matieres', verifyToken, async (req: any, res: any) => {
  try {
...
router.post('/create-parent', verifyToken, async (req: any, res: any) => {
  try {
...
router.post('/create-student', verifyToken, async (req: any, res: any) => {
  try {
...
router.post('/create-teacher', verifyToken, async (req: any, res: any) => {
  try {
...
interface ExamRow {
  id: number;
  title: string;
  examDate: string;  // or Date if you cast it
  semester: string;
  subject: string;
  class_name: string;
}

router.get('/getexamens', async (req: Request, res: Response) => {
  try {
    const examsRaw: any[] = await sequelize.query(`
      SELECT
        e.num_examen AS id,
...
// Route: GET /api/getdevoirs
type DevoirRow = {
  id: number;
  title: string;
  deadline: string; // ISO string from DB
  description: string;
  subject: string;
  className: string;
};

router.get('/getdevoirs', async (req: Request, res: Response) => {
  try {
    const devoirsRaw: DevoirRow[] = await sequelize.query(`
      SELECT
        d.num_devoir AS id,
